   ...
   <%-- <link rel="stylesheet" href="bootstrap/bootstrap.min.css">
    ...
    <script type="text/javascript">
        function deleteTeaPunch(tnodate) {
            //用户安全提示
            if (confirm("您确定要删除吗?")){
                location.href = "${pageContext.request.contextPath}/DeptDeleteTeaPunchServlet?tnodate=" + tnodate;
            }
        }
...
<body>
    <br>
    <div class="pull-left">
        <form class="form-inline" action="${pageContext.request.contextPath}/DeptQueryTeaPunchByPageServlet?currentPage=1&rows=7" method="post">
    ...
    <div class="pull-right">
        <a class="btn btn-default" href="${pageContext.request.contextPath}/view/deptadmin/addteapunch.jsp">添加打卡</a>
        <a class="btn btn-danger" href="javascript:void(0);" id="delSelected">删除选中</a>
    </div>

    <form id="form" action="${pageContext.request.contextPath}/DeptDelSelectedTeaPunchServlet" method="post">
        ...
        <table class="table table-striped table-hover">
            ...
            <c:forEach items="${pageBean.arrayList}" var="teaPunch" varStatus="s">
                <tr>
                    ...
                    <td>
                        <a class="btn btn-default" href="${pageContext.request.contextPath}/DeptQueryTeaPunchByIdServlet?tno=${teaPunch.tno}&tpunchdate=${teaPunch.tpunchdate}">修改</a>
         ...
         <nav aria-label="Page navigation" class="pull-left">
             <ul class="pagination">
                 ...
                 <li>
                     ...
                     <a href="${pageContext.request.contextPath}/DeptQueryTeaPunchByPageServlet?currentPage=${pageBean.currentPage - 1}&rows=7&tno=${tno}&tname=${tname}&tpunchdate=${tpunchdate}" aria-label="Previous">
                 ...
                 <c:forEach begin="1" end="${pageBean.totalPage}" var="i">
                     <c:if test="${pageBean.currentPage == i}">
                         <li class="active"><a href="${pageContext.request.contextPath}/DeptQueryTeaPunchByPageServlet?currentPage=${i}&rows=7&tno=${tno}&tname=${tname}&tpunchdate=${tpunchdate}">${i}</a></li>
                     </c:if>
                     <c:if test="${pageBean.currentPage != i}">
                         <li><a href="${pageContext.request.contextPath}/DeptQueryTeaPunchByPageServlet?currentPage=${i}&rows=7&tno=${tno}&tname=${tname}&tpunchdate=${tpunchdate}">${i}</a></li>
                     </c:if>
                 ...
                 <li>
                     ...
                     <a href="${pageContext.request.contextPath}/DeptQueryTeaPunchByPageServlet?currentPage=${pageBean.currentPage + 1}&rows=7&tno=${tno}&tname=${tname}&tpunchdate=${tpunchdate}" aria-label="Next">
